...
pub type RlweAutoKey = RlweKeySwitchingKey;
...
impl Rlwe {
    ...
    pub fn ak_gen(param: &RlweParam, sk: &RlweSecretKey, rng: &mut impl RngCore) -> RlweAutoKey {
        let sk_auto = sk.0.automorphism(Rlwe::AUTO_K);
        Self::ksk_gen(param, sk, &RlweSecretKey(sk_auto), rng)
    }
    ...
    pub fn automorphism(
        ...
        let ct_auto = ct.automorphism(Rlwe::AUTO_K);
...
pub(crate) mod test {
    ...
    pub(crate) fn testing_n_q(
        ...
        log_n_range.flat_map(move |log_n| {
            two_adic_primes(log_q, log_n + 1)
                .map(move |q| (1 << log_n, q))
                .take(10)
        })
    }
    ...
    fn key_switch() {
        ...
        for (n, q) in testing_n_q(log_n_range, log_q) {
            let param0 = RlweParam::new(q, p, n);
            let param1 = RlweParam::new(q, p, n).with_decomposor(log_b, k);
    ...
    fn automorphism() {
        ...
        let (log_n_range, log_q, p, log_b, k) = (0..10, 45, 1 << 4, 5, 9);
        for (n, q) in testing_n_q(log_n_range, log_q) {
            let param = RlweParam::new(q, p, n).with_decomposor(log_b, k);
            let (sk, pk) = Rlwe::key_gen(&param, &mut rng);
            let ak = Rlwe::ak_gen(&param, &sk, &mut rng);
            let m = Poly::sample_fq_uniform(n, p, &mut rng);
            let pt = Rlwe::encode(&param, &m);
            let ct0 = Rlwe::pk_encrypt(&param, &pk, &pt, &mut rng);
            let ct1 = Rlwe::automorphism(&param, &ak, &ct0);
            assert_eq!(
                m.automorphism(Rlwe::AUTO_K),
