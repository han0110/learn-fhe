...
use util::{bit_reverse, powers, AVec, BigFloat, Butterfly, Complex, DiagSparseMatrix, Zq};

// Algorithm 1 in 2018/1043
pub(crate) fn sfft(mut z: AVec<Complex>) -> AVec<Complex> {
...
pub(crate) fn sifft(mut z: AVec<Complex>) -> AVec<Complex> {
    assert!(z.len().is_power_of_two());
    ...
    bit_reverse(&mut z);
    let n = BigFloat::from(z.len());
    z.iter_mut().for_each(|z| *z /= &n);
    z
}

static POW5_TWIDDLE: OnceLock<Mutex<(Vec<usize>, Vec<Complex>)>> = OnceLock::new();

struct W<'a>(MutexGuard<'a, (Vec<usize>, Vec<Complex>)>, usize, bool);
...
impl<'a> W<'a> {
    ...
    fn iter(&self) -> impl Iterator<Item = &Complex> + Clone {
...
fn w<'a>(n: usize) -> W<'a> {
    ...
    if guard.0.len() < n {
        ...
        guard.1 = {
            let phase = BigFloat::pi() / BigFloat::from(2 * n);
...
pub(crate) fn sfft_fmats(n: usize) -> Vec<DiagSparseMatrix<Complex>> {
    ...
    (0..log_n)
        .map(|log_k| {
            ...
            let [zero, one] = [0, 1].map(|v| repeat(Complex::new(v.into(), 0.into())).take(m));
            let w = w(2 * m);
